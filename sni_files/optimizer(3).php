...
/**
 * 팝업창 닫기 선택시 그에 따른 쿠키값 저장 스크립트
 *
 * @todo 아래의 반복문이 3번 돌아가는 부분은 한번으로 처리가 가능한데 머리가 굳어버렸나 하기 구찮다.
 * @author 정상두
 */
$(document).ready(function(){
    // '직접만든 팝업 선택하기' 를 선택한 미리보기 모드인지 판단
    var chkPrv = location.toString().match(/mode=preview/i);

    $('#popup_close_btn').click(function() {
        if ($('#popup_close_check').attr('checked') == true) {

            var oCheckedData = splitKeyValue($('#popup_close_check').attr('value'));
            var aPopupCookie = cookieToArray($.cookie('SDE_POPUP'));

            var bCookie = false;

            for (var i=0; i<aPopupCookie.length; i++) {
                if (oCheckedData.key == aPopupCookie[i].key) {
                    aPopupCookie[i].value = oCheckedData.value;
                    bCookie = true;
                }
            }

            if (bCookie === false) {
                aPopupCookie[aPopupCookie.length] = oCheckedData;
            }


            var aSaveCookie = [];
            for (var i=0; i<aPopupCookie.length; i++) {
                aSaveCookie[i] = aPopupCookie[i].key+'='+aPopupCookie[i].value;
            }


            $.cookie('SDE_POPUP', aSaveCookie.join('&'), { expires: 1, path: '/'});
        }

        if (POPUP_TYPE == 'L') {
            var oNode = $(window.parent.document.body).find('#popup_'+POPUP_IDX);
            //setTimeout(function() { oNode.remove(); }, 100);
            //ECHOSTING-32220 IE10문제로 hide로 수정함
    ...
    // '직접만든 팝업 선택하기' 를 선택한 경우에는 관리자창의 '창닫기' 디자인내용을 가져오지 않는다.
    if (chkPrv != null) {//ECHOSTING-85488
        //// 팝업 미리보기 (직접만든 팝업 선택하기시 현재 설정의 footer 넣어주기)
         var sFooterId = '#foot_design_preview' ;//ECHOSTING-85488
        // 윈도우 팝업
        if (opener != undefined && opener.$('#frmNewPopup [name="foot_design_type"]:checked').val() != undefined) {
            //var sFooterId = '#foot_design_preview_' + ((opener.$('#frmNewPopup [name="foot_design_type"]:checked').val() == 'B') ? 'base' : 'detail');
            $('.xans-popup-footer').html(opener.$(sFooterId)[0].outerHTML);
        }
        // 레이어 팝업
        if (parent.parent.$('#frmNewPopup [name="foot_design_type"]:checked').val() != undefined) {
            //var sFooterId = '#foot_design_preview_' + ((parent.parent.$('#frmNewPopup [name="foot_design_type"]:checked').val() == 'B') ? 'base' : 'detail');
            $('.xans-popup-footer').html(parent.parent.$(sFooterId)[0].outerHTML);
        }
    }


    function cookieToArray(sCookie)
    {
        var aCookies = [];

        if (!sCookie) return aCookies;
        var _aCookies = new String(sCookie).split('&');



        for (var i=0; i<_aCookies.length; i++) {
            aCookies[i] = splitKeyValue(_aCookies[i]);
        }
        return aCookies;
    }

    function splitKeyValue(sData)
    {
        var aData = new String(sData).split('=');
        return {key:aData[0], value:aData[1]};
    }
});
